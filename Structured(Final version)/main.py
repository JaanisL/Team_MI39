...
class GameApp:
    ...
    def visualize_game_tree(self):
        ...
        tree = Tree(self.node.sequence)
        tree.generate_tree(tuple(self.node.sequence))
        G = tree.get_graph()
        
        levels = {node: data['level'] for node, data in G.nodes(data=True)}
        pos = {node: (i, -level) for i, (node, level) in enumerate(levels.items())}
        
        nx.draw(G, pos, with_labels=True, node_color='lightblue', ax=ax, font_size=8, edge_color='gray')
