...
class GameApp:
    ...
    def create_buttons(self):
        ...
        for i in range(len(self.node.sequence) - 1):
            btn = tk.Button(
                self.frame, 
                text=f"{self.node.sequence[i]} {self.node.sequence[i+1]}", 
    ...
    def ai_move(self):
        if len(self.node.sequence) > 1:
            ...
            if best_move is not None:
                chosen_pair = f"{self.node.sequence[best_move]} {self.node.sequence[best_move+1]}"
            
                selected_button = None
                for widget in self.frame.winfo_children():
                    if widget.cget("text") == chosen_pair:
                ...
                if selected_button:
                    ...
                    self.root.after(1000, lambda: selected_button.config(bg="SystemButtonFace"))
                    self.root.after(1000, lambda: self.make_move(best_move))
